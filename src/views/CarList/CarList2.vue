<template>
  <div class="box">
    ...
    <div class="boxTwo">
      ...
      <div class="pass">
          <el-button type="success" size="small" @click="PassAll()">本页全部通过</el-button>
      </div>
    </div>
    <el-table
      :data="list"
    ...
    >
      ...
      <el-table-column align="center" prop="driverLicencePictureMain" label="驾驶证">
        <template slot-scope="scope">
            ...
            <img @click="rotate" :style="{transform:'rotateZ('+ deg +'deg)',width: '500px',height: '350px'}" :src="scope.row.driverLicencePictureMain"/>
      ...
      <el-table-column align="center" prop="drivingLicensePictureMain" label="行驶证">
          <template slot-scope="scope">
              ...
              <img @click="rotate" :style="{transform:'rotateZ('+ deg +'deg)',width: '500px',height: '350px'}" :src="scope.row.drivingLicensePictureMain"/>              
...
export default {
  ...
  methods: {
    ...
    PassAll() {
      ...
      that.$http.post(url.batchCheckDriver, {
        token: token,
        driverPhones: that.passChe,
        ...
        if(data.body.status==200){
          that.$http.post(url.getDriverCheckList, {
            ...
            for (var i = 0; i < params.length; i++) {
              that.passChe.push(params[i].phone);
            }
          });
        } 
      });
