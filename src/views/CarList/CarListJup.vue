  ...
  <div class="box">
    ...
    <div class="boxCenter">
      <div class="boxPictures">
        <img @click="rotate" :style="{transform:'rotateZ('+ deg +'deg)'}" :src="params.driverLicencePictureMain"/>
      </div>
      <div class="boxPictures">
        <img @click="rotate1" :style="{transform:'rotateZ('+ deg1 +'deg)'}" :src="params.drivingLicensePictureMain"/>
      </div>
    </div>
    <div class="Texzt2">
    ...
    <template>
      <div class="boxShoeH">
          <el-button type="danger" round @click="notPass(params.phone)">拒绝</el-button>
        <template>
          <el-button type="success" round @click="auditPass(params.phone)">通过</el-button>
        </template>
      </div>
    </template>
  </div>
...
export default {
  ...
  methods: {
    auditPass(phone) {
      ...
      var that = this;
      var token = JSON.parse(sessionStorage.getItem('token'));
      that.$http.post(url.checkDriver, {
          token : token,
          status: 4,
          driverPhone: phone,
        }).then(function(data) {
          ...
          if(data.body.params.licenseGetDate){
            that.params = data.body.params;
            that.time = dateFormatter(that.$route.params.time);//发布时间
            that.drivingRegDate = dateFormatter(data.body.params.drivingRegDate);//行驶证注册时间
            that.licenseGetDate = dateFormatter(data.body.params.licenseGetDate);//驾驶证获得时间
